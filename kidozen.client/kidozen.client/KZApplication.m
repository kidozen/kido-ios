...
-(void) sendByServiceBus:(NSMutableURLRequest *) request completion:(void (^)(KZResponse *))block
{
    ...
    [NSURLConnection sendAsynchronousRequest:request queue:NULL completionHandler:^(NSURLResponse * response, NSData * data, NSError * error) {
        KZResponse * r = [[KZResponse alloc] initWithResponse:response urlResponse:NULL andError:error];
        block(r);
    }];
}
